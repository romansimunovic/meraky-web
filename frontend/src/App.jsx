...
export default function App() {
  ...
  const fetchServices = async () => {
    try {
      ...
      setActiveTab((prev) => {
        ...
        return prev >= normalized.length ? 0 : prev;
      });
    } catch (err) {
  ...
  const handleQuickBook = (category, itemName) => {
    ...
    window.location.href = '#rezervacija';
  };
...
fetchServices();
  ...
  return (
    <div className="min-h-screen bg-brand-cream font-sans-clean text-brand-espresso selection:bg-brand-moss selection:text-brand-espresso overflow-x-hidden flex flex-col justify-between">
      <main>
        ...
        <section id="tretmani" className="max-w-6xl mx-auto px-4 sm:px-6 py-16 border-t border-brand-sand/60">
          <div className="text-center max-w-xl mx-auto mb-12">
            ...
            <p className="text-sm sm:text-base text-brand-espresso/75 mt-2">
              Svakom tretmanu pristupamo individualno uz vrhunsku kozmetiku i holistički pristup.
            </p>
          </div>

          {loadingServices ? (
            <div className="text-center py-12 text-brand-espresso/70">Učitavam cjenik iz baze...</div>
          ) : servicesData.length === 0 ? (
            <div className="text-center py-12 text-brand-espresso/70">Trenutno nema dostupnih usluga.</div>
          ) : (
            <>
              ...
              <div className="bg-white rounded-3xl border border-brand-sand p-6 sm:p-10 max-w-3xl mx-auto shadow-sm">
                <div className="space-y-6">
                  {servicesData[activeTab]?.items.map((item) => (
                    <div
                      key={item.id}
                      className="flex flex-col sm:flex-row sm:justify-between sm:items-center py-4 border-b border-brand-clay last:border-0 gap-3 sm:gap-4"
                    >
                      <div className="space-y-1">
                        <h4 className="text-sm sm:text-base font-bold text-brand-espresso">{item.name}</h4>
                        <p className="text-xs text-brand-espresso/60">Trajanje: {item.duration || '30 min'}</p>
                      </div>
